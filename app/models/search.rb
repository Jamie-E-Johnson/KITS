  ...
  def custom_search
    ...
    if self.search_attributes.size > 0
      ...
      Search.last.search_attributes.each do |it|
        if filter.empty?
          filter = Intern.joins(:internships).where("lower(#{it.drop_down}) like lower(?)", "%#{it.criteria}%")
        else
          filter = filter.joins(:internships).where("lower(#{it.drop_down}) like lower(?)", "%#{it.criteria}%")
        end
      end
    else
